...
# How to setup GTG PubSub server
...
Now, you can use your favorite XMPP client to connect to your own
*localhost* server using your *username* and *password*.

You have to add configuration for *max_items_node* to a big number to
synchronize all of your tasks:

`sudo sed -i 's/{mod_pubsub,.\*/&\\n\\t\\t{max_items_node, 9999},/'/etc/ejabberd/ejabberd.cfg sudo service ejabberd restart`

There is my complete */etc/ejabberd/ejabberd.cfg* configuration file. It
is only a slightly modified standard Debian configuration.
...
-   [ejabberd tutorials](http://www.ejabberd.im/tutorials)
-   [x60br](http://x60br.berlios.de/) - graphical manager of PubSub
